...
<xsl:stylesheet 
  ...
  <xd:doc scope="stylesheet">
    <xd:desc><xd:p>This stylesheet transforms a Medication Process 9 XML file with therapeutic building
      ...
      <xd:p>The input XML is based on the AORTA 9.0.3 specification.</xd:p>
  ...
  <xsl:template name="addHtmlHead">
    <head>
      ...
      <title>Viewer MP9 -
        <xsl:value-of select="(tokenize(document-uri(/),'/'))[last()]"/> <!-- Add the filename of the input file -->
      </title>
  ...
  <xsl:function name="nf:determineDocumentType" as="xs:decimal">
    ...
    <xsl:choose>
      <xsl:when test="($topNode//organizer/templateId[@root='2.16.840.1.113883.2.4.3.11.60.20.77.10.9132'] or 
                       $topNode//ClinicalDocument/templateId[@root='2.16.840.1.113883.2.4.3.11.60.20.77.10.9146'])">
          <xsl:value-of select="$DOCTYPE_MO"/>
        </xsl:when>
      <xsl:when test="($topNode//organizer/templateId[@root='2.16.840.1.113883.2.4.3.11.60.20.77.10.9104'] or 
                       $topNode//ClinicalDocument/templateId[@root='2.16.840.1.113883.2.4.3.11.60.20.77.10.9133'])">
          <xsl:value-of select="$DOCTYPE_MEDDATA"/>
        </xsl:when>
      <xsl:when test="($topNode//organizer/templateId[@root='2.16.840.1.113883.2.4.3.11.60.20.77.10.9125'] or 
  ...
  <xsl:function name="nf:determineStartDateTime" as="xs:dateTime">
    ...
    <xsl:variable name="isStop" as="xs:boolean" 
      select="exists($componentNode/entryRelationship/observation[templateId/@root='2.16.840.1.113883.2.4.3.11.60.20.77.10.9067'])"/>
  ...
  <xd:doc>
    <xd:desc>
      <xd:p>Returns the start date (and time) of the most recent component of the specified
        type.</xd:p><xd:p>(Note: this is later used to find the most recent node by comparing the start
    ...
    <xsl:variable name="mostRecentComponent" as="xs:dateTime*">  
      <xsl:choose>
        <xsl:when test="$componentType eq $CT_MA">
          <xsl:for-each select="$componentNode[hl7:templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9148']" >
            <xsl:value-of select="nf:determineStartDateTime(.)"/>
          </xsl:for-each>
          <!-- Note that there is also a MA template 9185, but that is only used for proposal messages, 
               which this viewer doesn't support -->
        </xsl:when>
        <xsl:when test="$componentType eq $CT_TA">
          <xsl:for-each select="$componentNode[hl7:templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9152']" >
            <xsl:value-of select="nf:determineStartDateTime(.)"/>
          </xsl:for-each>
        </xsl:when>
  ...
  <xsl:function name="nf:determineMBHstartDateTime" as="xs:dateTime">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:value-of select="$nfInvalidDateTime"/>  <!-- Shouldn't happen: author/effectiveTime is required -->
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>
  ...
  <xsl:function name="nf:determineMBHstopDate" as="xs:dateTime">
    ...
    <xsl:variable name="mbhStopDates" as="xs:dateTime*">
      <xsl:for-each select="$curGroup">
        ...
        <xsl:if test="(($componentType eq $CT_MA) and ($startDate eq $mostRecentMAdateTime)) or
  ...
  <xsl:function name="nf:determineMBHtable" as="xs:decimal">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$mbhStopDate ne $nfFutureDateTime and 
      ...
      <xsl:when test="$mbhStopDate ne $nfInvalidDateTime and
      ...
      <xsl:when test="$mbhStartDate ne $nfInvalidDateTime and 
  ...
  <xsl:template name="createTableHeader">
    ...
    <tr>
      <th width="3%"></th>  <!-- collapse MBH button row -->
      <th width="3%"></th>  <!-- collapse Type button row -->
      <th width="3%">
  ...
  <xsl:template name="DisplayMBH">
    ...
    <xsl:variable name="mbhContent" select="$curContext//*[*/*[hl7:id/concat(@root, '#', @extension) = $mbhId]]"/>
    ...
    <tr bgcolor="#EEEEEE" id="debugInfo" class="hideDebugInfo">
      ...
      <td><xsl:value-of select="if (xs:dateTime(@startdatum) != $nfInvalidDateTime) then @startdatum else ''"/></td>
      <td><xsl:value-of select="if (xs:dateTime(@stopdatum) != $nfFutureDateTime) then @stopdatum else ''"/></td>
      <td colspan="6"></td>
    </tr>

    <!-- TODO: don't count nullified components for nrComponentsOfMBH, 
               otherwise a '+' may be shown in the 1st column when there is only one component -->
    <xsl:variable name="nrComponentsOfMBH" select="count($mbhContent/parent::hl7:entry | $mbhContent/parent::hl7:component)"/>
    ...
    <xsl:variable name="firstComponentType" select="if ($mostRecentMAdateTime ne $nfInvalidDateTime) then $CT_MA else
      if ($mostRecentTAdateTime ne $nfInvalidDateTime) then $CT_TA else $CT_GB"/>
    
    <!-- Display all MA's, except those that are nullified -->
    <xsl:variable name="nrMAofMBH" select="count($mbhContent[hl7:templateId[@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9148'] and not(hl7:statusCode[@code='nullified'])])"/>
    <xsl:for-each select="$mbhContent[hl7:templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9148' and not(hl7:statusCode[@code='nullified'])]" >
      <xsl:sort select="nf:determineStartDateTime(.)" order="descending"/>
        <xsl:call-template name="displayComponent">
          ...
          <xsl:with-param name="nrComponentsOfType" select="$nrMAofMBH"/>
    ...
    <xsl:variable name="nrTAofMBH" select="count($mbhContent[hl7:templateId[@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9152'] and not(hl7:statusCode[@code='nullified'])])"/>
    <xsl:for-each select="$mbhContent[hl7:templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9152' and not(hl7:statusCode[@code='nullified'])]" >
      <xsl:sort select="nf:determineStartDateTime(.)" order="descending"/>
      <xsl:call-template name="displayComponent">
        <xsl:with-param name="curMBHid" select="$curMBH"/>
        <xsl:with-param name="nrComponentsOfMBH" select="$nrComponentsOfMBH"/>
        <xsl:with-param name="nrComponentsOfType" select="$nrTAofMBH"/>
    ...
    <xsl:for-each select="$mbhContent[hl7:templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9154' or 
      ...
      <xsl:call-template name="displayComponent">
        <xsl:with-param name="curMBHid" select="$curMBH"/>
        <xsl:with-param name="nrComponentsOfMBH" select="$nrComponentsOfMBH"/>
        <xsl:with-param name="nrComponentsOfType" select="$nrGBofMBH"/>
  ...
  <xsl:template name="displayComponent">
    ...
    <xsl:variable name="isDeviatingGB" as="xs:boolean"
      ...
      ($substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9117']/value/@value eq 'false') else false()"/>
    ...
    <tr class="{$curMBHid}" id="{$componentType}">
      ...
      <td>
        <xsl:call-template name="getMedicationName"><xsl:with-param name="substAdmNode" select="$substAdmNode"/></xsl:call-template>
        <xsl:call-template name="showReferences"><xsl:with-param name="substAdmNode" select="$substAdmNode"/></xsl:call-template>
      </td>
      <td>
      ...
      <td>
        <xsl:call-template
          name="getMedicationEndDateOrDuration"><xsl:with-param name="substAdmNode" select="$substAdmNode"/></xsl:call-template>
      </td>
      <td>
        <xsl:call-template
          name="getMedicationDosage"><xsl:with-param name="substAdmNode" select="$substAdmNode"/></xsl:call-template>
      </td>
      <td>
        <xsl:call-template
          name="getMedicationRoute"><xsl:with-param name="substAdmNode" select="$substAdmNode"/></xsl:call-template>
      </td>
      <td>
        <xsl:call-template
          name="getMedicationReason"><xsl:with-param name="substAdmNode" select="$substAdmNode"/></xsl:call-template>
      </td>
      <td>
        <xsl:call-template
          name="getMedicationRemark"><xsl:with-param name="substAdmNode" select="$substAdmNode"/></xsl:call-template>
      </td>
      <td>
        <xsl:call-template
          name="getMedicationAuthor"><xsl:with-param name="substAdmNode" select="$substAdmNode"/></xsl:call-template>
      </td>
    </tr>
  </xsl:template>
  ...
  <xsl:function name="nf:determineComponentType" as="xs:decimal">
    ...
    <xsl:variable name="componentType">
      <xsl:choose>
        <xsl:when test="$substAdmNode/code/@codeSystem = '2.16.840.1.113883.6.96' and   
          $substAdmNode/code/@code = '16076005'">
          <xsl:value-of select="$CT_MA"/>
        </xsl:when>
        <xsl:when test="$substAdmNode/code/@codeSystem = '2.16.840.1.113883.6.96' and   
          $substAdmNode/code/@code = '422037009'">
          <xsl:value-of select="$CT_TA"/>
        </xsl:when>
        <xsl:when test="$substAdmNode/code/@codeSystem = '2.16.840.1.113883.2.4.3.11.60.20.77.5.3' and   
          $substAdmNode/code/@code = '6'">
  ...
  <xsl:template name="getComponentTypeIcon">
    <xsl:param name="componentType" as="xs:decimal"></xsl:param>
    
    <xsl:choose>
      <xsl:when test="$componentType eq $CT_MA"><img src="{$ImagedataIconCareProvider}" title="{nf:getLocalizedString('medicationAgreement')}"/></xsl:when>
  ...
  <xsl:template name="getMedicationName">
    ...
    <xsl:value-of
      select="$substAdmNode/consumable/manufacturedProduct/manufacturedMaterial/code/@displayName" />
    <xsl:value-of
      select="$substAdmNode/consumable/manufacturedProduct/manufacturedMaterial/name" />
    ...
    <div id="debugInfo" class="hideDebugInfo">
      ...
      <xsl:value-of
        select="$substAdmNode/consumable/manufacturedProduct/manufacturedMaterial/pharm:desc" />
    </div>
  </xsl:template>
  ...
  <xsl:template name="showReferences">
    ...
    <xsl:variable name="curMBHid" select="concat($substAdmNode/entryRelationship/procedure[templateId[@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9084']]/id/@root, 
    ...
    <xsl:for-each select="$substAdmNode/entryRelationship[@typeCode='REFR']/substanceAdministration[./templateId/@root eq '2.16.840.1.113883.2.4.3.11.60.20.77.10.9086']">
      ...
      <xsl:variable name="refMBHid" select="concat($referredMA[1]/entryRelationship/procedure[templateId[@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9084']]/id/@root, 
        $referredMA[1]/entryRelationship/procedure[templateId[@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9084']]/id/@extension)"/>
      <xsl:variable name="refTitle">
        <xsl:value-of
          select="$referredMA/consumable/manufacturedProduct/manufacturedMaterial/code/@displayName" />
        <xsl:value-of
          select="$referredMA/consumable/manufacturedProduct/manufacturedMaterial/name" />
      </xsl:variable>
    ...
    <xsl:for-each select="$substAdmNode/entryRelationship[@typeCode='REFR']/substanceAdministration[./templateId/@root eq '2.16.840.1.113883.2.4.3.11.60.20.77.10.9101']">
      ...
      <xsl:variable name="refMBHid" select="concat($referredTA[1]/entryRelationship/procedure[templateId[@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9084']]/id/@root, 
        $referredTA[1]/entryRelationship/procedure[templateId[@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9084']]/id/@extension)"/>
      <xsl:variable name="refTitle">
        <xsl:value-of
          select="$referredTA/consumable/manufacturedProduct/manufacturedMaterial/code/@displayName" />
        <xsl:value-of
          select="$referredTA/consumable/manufacturedProduct/manufacturedMaterial/name" />
      </xsl:variable>
    ...
    <xsl:for-each select="$substAdmNode/entryRelationship[@typeCode='REFR']/substanceAdministration[./templateId/@root eq '2.16.840.1.113883.2.4.3.11.60.20.77.10.9176']">
      ...
      <xsl:variable name="refMBHid" select="concat($referredGB[1]/entryRelationship/procedure[templateId[@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9084']]/id/@root, 
        $referredGB[1]/entryRelationship/procedure[templateId[@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9084']]/id/@extension)"/>
      <xsl:variable name="refTitle">
        <xsl:value-of
          select="$referredGB/consumable/manufacturedProduct/manufacturedMaterial/code/@displayName" />
        <xsl:value-of
          select="$referredGB/consumable/manufacturedProduct/manufacturedMaterial/name" />
      </xsl:variable>
  ...
  <xsl:template name="getMedicationStartDate">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$registerAuthorDate">    <!-- Always show the registration date if the start date is not specified -->
        <img src="{$ImagedataIconPrescribe}" height="16px" title="{$tooltip}"/> <xsl:text> </xsl:text>
        <xsl:value-of select="nf:printHl7DateTime($registerAuthorDate, true())" />
      </xsl:when>
      <!-- Pre 9.04 compatibility -->
      <xsl:when test="$registerPatientDate">    <!-- Always show the registration date if the start date is not specified -->
        <img src="{$ImagedataIconPrescribe}" height="16px" title="{nf:getLocalizedString('registrationDate')}"/> <xsl:text> </xsl:text>
    ...
    <div id="debugInfo" class="hideDebugInfo">
      ...
      <xsl:if test="$substAdmNode/effectiveTime/@value or $substAdmNode/effectiveTime/low">
        <xsl:if test="$registerAuthorDate">    <!-- Always show the registration date if the start date is not specified -->
          <img src="{$ImagedataIconPrescribe}" height="16px" title="{$tooltip}"/> <xsl:text> </xsl:text>
          <xsl:value-of select="nf:printHl7DateTime($registerAuthorDate, true())" />
        </xsl:if>
        <!-- Pre 9.04 compatibility -->
        <xsl:if test="$registerPatientDate">    <!-- Always show the registration date if the start date is not specified -->
          <img src="{$ImagedataIconPrescribe}" height="16px" title="{nf:getLocalizedString('registrationDate')}"/> <xsl:text> </xsl:text>
          <xsl:value-of select="nf:printHl7DateTime($registerPatientDate, true())" />
        </xsl:if>
      </xsl:if>
  ...
  <xsl:template name="getMedicationReason">
    ...
    <xsl:if
      test="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9160']">
      ...
      <xsl:value-of
        select="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9160']/value/@displayName"/>
      <xsl:value-of
        select="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9160']/value/originalText"
      />
    </xsl:if>
    ...
    <xsl:if
      test="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9114']">
      <xsl:value-of
        select="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9114']/text"/>
    </xsl:if>
    ...
    <xsl:if test="
      ...
      <br/>
    </xsl:if><!-- 'reden van staken': found in 'reden van afspraak' different locations for ma, ta and gb. -->
    <xsl:if
      test="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9067']">
      <xsl:variable name="stopType"
        select="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9067']/value/@code"
        as="xs:decimal"/>
      ...
      <xsl:value-of
        select="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9068']/value/@displayName"/>
      <xsl:value-of
        select="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9083']/text"/>
      <xsl:value-of
        select="$substAdmNode/entryRelationship/observation[templateId/@root = '2.16.840.1.113883.2.4.3.11.60.20.77.10.9115']/value/@displayName"/>
    </xsl:if>
  ...
  <xsl:template name="getMedicationRemark">
    ...
    <xsl:if test="exists($substAdmNode/entryRelationship/act[templateId/@root='2.16.840.1.113883.2.4.3.11.60.20.77.10.9069']) and
    ...
    <xsl:choose>
      ...
      <xsl:when test="$substAdmNode/author/assignedAuthor/assignedPerson">
       <xsl:call-template name="util:show-name">
        ...
        <xsl:value-of select="if ($substAdmNode/author/assignedAuthor/code) then 
          concat(' (', $substAdmNode/author/assignedAuthor/code/@displayName, ')') else 
          '' "/>
      </xsl:when>
